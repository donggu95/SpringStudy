...
<%
	...
	// 데이터 전송을 위해서 자바의 객체를 json 문자열로 전송해야 하는데, 이 json문자열로 변환하는 작업이 너무 효율성이 떨어짐...
	// 따라서 외부 라이브러리에 의존해서 효율성을 높여보자!!
	
	// 문자열이 아닌 json 객체를 자동으로 생성하기 !! {필요X}
	JsonObject json = new JsonObject();  // {}가 만들어졌다 !
	json.addProperty("store_id", store.getStore_id());   // "store_id": 1
	json.addProperty("title", store.getTitle());
	json.addProperty("sido", store.getSido());
	json.addProperty("score", store.getScore());
	
	Gson gson = new Gson();  //객체와 문자열간 자동변환을 담당 
	String jsonStr = gson.toJson(json);  // 객체를 -> 문자열로 변환 
	out.print(jsonStr);
